...
bool canMove(vector<string> &map, int x, int y)
{
  ...
  // The cell is walkable if it's '.' or '*' (we handle '*' separately)
...
int solve(int startx, int starty, int endx, int endy, vector<string> &map)
{
  ...
  while (!q.empty())
  {
    ...
    for (auto &d : arr)
    {
      ...
      if (canMove(map, newr, newc) && dist[newr][newc][dig] == 1e9)
      {
        ...
        {
          // Attempt to dig in the current direction
          int rr = newr + d.first;
          int cc = newc + d.second;
          // Check if the position after digging is within bounds and walkable
          if (rr >= 0 && rr < Row && cc >= 0 && cc < Colmun && map[rr][cc] == '.' && dist[rr][cc][1] == 1e9)
          {
            dist[rr][cc][1] = dist[r][c][dig] + 2; // +1 for digging, +1 for moving
            q.push({rr, cc, 1});
          }

          // Alternatively, attempt to dig horizontally relative to current direction
          rr = newr; // Same row
          cc = newc + d.second;
  ...
  // After BFS, determine the minimum distance to the destination
...
signed main()
{
  ...
  // Read grid dimensions: Colmun (number of columns) first, then Row (number of rows)
  cin >> Colmun >> Row;

  // Read the grid map
  vector<string> map(Row);

  // Read starting and ending positions
