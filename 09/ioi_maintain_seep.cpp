...
public:
  ...
  int kruskal()
  /*
    รู้ไงว่ามันเชื่อมกันไม่ทั้งหมด ถ้าเชื่อมกันทั้งหมดพิมพ์ ans
    ถ้าไม่เชื่อมกันทั้งหมดพิมพ์ -1
  */
  {
    heap = priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, greater<tuple<int, int, int>>>();
    path_mst.clear();
    ans = 0;
    fill(all(parent), -1);
